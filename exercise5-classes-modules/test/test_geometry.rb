...
class TestMeasurable < Minitest::Test
  def test_circle_includes_measurable
    assert Geometry::Circle.ancestors.include?(Geometry::Measurable)
  end

  def test_rectangle_includes_measurable
    assert Geometry::Rectangle.ancestors.include?(Geometry::Measurable)
  end

  def test_larger_than
    big   = Geometry::Circle.new(10.0)
    small = Geometry::Circle.new(1.0)
    assert big.larger_than?(small)
    refute small.larger_than?(big)
  end
end
