...
- **Exercises 1–7**: C code includes `ruby.h`, uses `VALUE` types, defines `Init_`, compiles into a `.bundle`/`.so` that Ruby treats as a native extension.
- **Exercise 8**: C code is a plain C library — no `ruby.h`, no `VALUE`, no `Init_`. The `ffi` gem uses `libffi` to call its functions at runtime. No Ruby-specific compilation.

## What's Provided

`mylib.c` — the C source;
`Makefile` - This is a basic Makefile. Very different from the ones generated by extconf.rb

## Steps

1. Open `mylib.c` — note: no `#include <ruby.h>`, no `VALUE`, no `Init_` function
2. Run `make` from the `exercise5-ffi/` directory
3. Write the FFI bindings in `runner.rb` — see FFI instructions below
4. Run `ruby runner.rb`

## Writing the Makefile (already written)
